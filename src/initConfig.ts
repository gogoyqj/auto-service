...
const schema = {
  properties: {
    ...
    swaggerVersion: {
      description:
        '请输入 swagger 接口文档版本，例如：v2、v3，默认 v2（如果接口文档类型是 yapi 则可直接忽略）',
      required: false
    },
    outputType: {
      description:
        '请输入 service 代码输出类型，例如：services（生成类型和接口调用代码）, types（仅生成类型）, global（生成通过同一个全局方法重载实现类型化调用的代码），默认 services',
      required: false
    },
    output: {
...
export default function initConfig(ConfigFile: string) {
  ...
  prompt.get(schema, function(err, result) {
    ...
    if (
      ['v2', 'v3'].indexOf(swaggerVersion) !== -1 ||
      ['services', 'types', 'global'].indexOf(outputType) !== -1
    ) {
      defaultConfigJSON.swaggerParser!['-t'] = `${swaggerVersion === 'v3' ? 'v3/' : ''}plugins/${
        outputType === 'global'
          ? 'typescript-tkit-autos'
          : outputType === 'types'
          ? 'types-only'
          : 'typescript-tkit'
      }`;
    ...
    if (ConfigFile.endsWith('.ts')) {
      fs.writeFileSync(
        ConfigFile,
        [
          `import { JSON2Service } from "auto-service/lib/consts"`,
          ``,
          `const config: JSON2Service = ${JSON.stringify(defaultConfigJSON, null, 2)}`,
          ``,
