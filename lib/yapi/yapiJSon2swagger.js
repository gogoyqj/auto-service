...
function yapiJSon2swagger(list, yapiConfig) {
  var basePath = '';
  var info = {
    title: 'unknown',
    version: 'last',
  ...
  var reg = basePath ? new RegExp('^' + basePath) : undefined;
  var swaggerObj = {
    swagger: '2.0',
    info: info,
    basePath: basePath,
    ...
    paths: (function() {
      ...
      var _loop_1 = function(aptTag) {
        var _loop_2 = function(api) {
          // list of api
          var url = reg ? api.path.replace(reg, '') : api.path;
          if (apisObj[url] == null) {
            apisObj[url] = {};
          }
          apisObj[url][api.method.toLowerCase()] = (function() {
            var apiItem = {};
                      ...
                      if (!jsonParam['type']) {
                        ...
                        if (yapiConfig && yapiConfig.required) {
                          jsonParam = JSON.parse(
                            JSON.stringify(jsonParam).replace(/"([^*"]+":)/g, function(all, name) {
                              return '"*' + name;
                            })
                          );
                        }
                    ...
                    if (api.res_body) {
                      ...
                      if (resBody !== null) {
                        if (resBody['type']) {
                          ...
                          if (yapiConfig && yapiConfig.required) {
                            resBody = JSON.parse(
                              JSON.stringify(resBody).replace(/"([^*"]+":)/g, function(all, name) {
                                return '"*' + name;
                              })
                            );
                          }
